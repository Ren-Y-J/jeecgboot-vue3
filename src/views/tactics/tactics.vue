<template>
			...
			>
				<template #bodyCell="{ column, record }">
					<template v-if="column.dataIndex === 'policiesEnable'">
						<div
							v-if="record.policiesEnable == true"
							style="text-align: center; display: flex; justify-content: center; align-items: center"
						>
							启用
						</div>
						<div
							v-if="record.policiesEnable == false"
							style="text-align: center; display: flex; justify-content: center; align-items: center"
						>
							未启用
						</div>
	...
	<div>
		<a-modal
			...
			:body-style="modalStyle"
			style="top: 200px"
			v-model:visible="visible_Time_edit"
			title="修改策略组"
			width="600px"
			@ok="edit_time"
		>
			...
			>
				<a-form-item label="启用周期" name="aclType" style="margin-top: 26px" class="lable_form">
					<a-checkbox-group
						v-model:value="formState_edit.policiesTimeType"
						name="policiesTimeType"
						style="width: 100%"
						@change="checkboxTime"
						class="radio_right"
...
const editGroup = (record)=>{
  	...
  	BackLine(record.policiesId).then((res)=>{//回显接口
		...
		let result = formState_edit.value.policiesTimeRange.split(',').map(item => item.trim());
		let transformedA = [];
...
const Policyedit = (evt)=>{
   ...
   formDataName_edit.value.forEach(item=>{   
     	dif_time =item.state_value_edit.toString()
