<template>
	<div class="mainset">
		<div class="nav">
			<a-form name="basic" :label-col="{ span: 4 }" :wrapper-col="{ span: 16 }" autocomplete="off"
				@finish="onFinish" @finishFailed="onFinishFailed">
				<a-row :gutter="1">
					<a-col :md="6" :sm="24">
						<a-form-item style="margin-bottom: 0px;" label="主机组名称" name="zone" :labelCol="{ span: 6 }"
							:wrapperCol="{ span: 16 }">
		...
		<div class="contaion">
			<a-card>
				...
				<div class="pagination">
					<a-pagination :show-total="totals => `共 ${totals} 条数据`" v-model:current="formData.pageNum"
						:total="totals" v-model:pageSize="formData.pageSize" show-size-changer
						@showSizeChange="onShowSizeChange" @change="changeFn" />
				</div>
			</a-card>
		</div>
		<div class="modal">
			...
			<a-modal v-model:visible="visible" :title="opTitle" @ok="addoreditFn" @cancel="onCloses">
				<a-form :model="rowData" ref='formRefmainset' name="basic" :label-col="{ span: 6 }"
